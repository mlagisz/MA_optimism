...
MA_random0 <- rma.mv(yi=d, V=Vd, 
                     random=list(~1|ArticleID, ~1|ExperimentID, ~1|GroupID, ~1|EffectID), 
                     dat=dat, method="REML") 
summary(MA_random0) ### estimate   0.26
calc.I2(dat$Vd, MA_random0) #83.5% total
save(MA_random0,file = "Rdata/MA_random0.Rdata")

######## random-effect model with VCV matrix
VCV <- make_VCV_matrix(data = dat, V = "Vd", cluster = "GroupID", obs = "EffectID")
MA_random0_VCV <- rma.mv(yi = d, V = VCV, 
                         random = list(~1|ArticleID, ~1|ExperimentID, ~1|GroupID, ~1|EffectID), 
                         data = dat, method = "REML")
summary(MA_random0_VCV) #0.22
calc.I2(dat$Vd, MA_random0_VCV) #89.2% total


######## random-effect model with ArticleID and species as random:
MA_random_species <- rma.mv(yi=d, V=Vd, random=list(~1|Species_Latin, ~1|ArticleID, ~1|ExperimentID, ~1|EffectID), data=dat, method="REML")  
summary(MA_random_species) ### species not contributing
...
calc.I2(dat$Vd, MA_random_species) #89% total
...
CorMatrix_lat_prop <- vcv(tree, corr=TRUE)
setdiff(colnames(CorMatrix_lat_prop), unique(dat$Species_Latin)) #ok

res_lat_prop_phylo <- rma.mv(yi=d, V=VCV, random=list(~1|Species_Latin, ~1|ArticleID, ~1|ExperimentID, ~1|EffectID), R=list(Species_Latin=CorMatrix_lat_prop), dat=dat, method="REML") 
summary(res_lat_prop_phylo) ### estimate  0.2187  CI  -0.0623  0.4997   
calc.I2(dat$Vd, res_lat_prop_phylo) #4% phylogeny
...
res_lat_prop_phylo2 <- rma.mv(yi=d, V=Vd, random=list(~1|ArticleID, ~1|ExperimentID, ~1|ScientificName, ~1|Species_Latin), R=list(Species_Latin=CorMatrix_lat_prop), dat=dat, method="REML") 
summary(res_lat_prop_phylo2) ### estimate  0.1676  CI  0.0349  0.3003  * #no contribution from phylogeny #caluclating I2 values
calc.I2(dat$Vd, res_lat_prop_phylo2) #4% phylogeny
...
res_species <- rma.mv(yi=d, V=Vd, mod = ~Species_Latin -1, random=list(~1|ArticleID), dat=dat, method="REML") 
...
res_lat_prop_phylo_MT <- rma.mv(yi=d, V=VCV, mod = ~MeasureType-1, random=list(~1|Species_Latin, ~1|ArticleID, ~1|ExperimentID, ~1|EffectID), R=list(Species_Latin=CorMatrix_lat_prop), dat=dat, method="REML") 
...
par(mfcol=c(1,1)) 
par(mar=c(4,4,2,1))
plot(dat$Hd, sqrt(1/dat$Vd), xlim=c(-6.5,6.5), xlab = "Effect size [Hd]", ylab="Precision [1/SE]", main="")
abline(v=0,lty=3)
dev.off()
```
...
par(mfrow=c(1,3))
par(mar=c(4,4,4,1))
plot(dat$Hd, sqrt(1/dat$Vd), xlim=c(-3,3), ylim=c(0,3), xlab = "effect size [Hd]", ylab="Precision [1/SE]", frame.plot=FALSE, main="A")
abline(v=0,lty=3)
